...
#### 📤 Giải thích chi tiết 3 định dạng xuất
...
**Ưu điểm:**
- ✅ **Tích hợp web map**: Leaflet, Mapbox, OpenLayers
- ✅ **Import GIS online**: QGIS Cloud, ArcGIS Online
- ✅ **API-friendly**: JSON dễ parse bằng JavaScript/Python
- ✅ **Nhẹ**: ~50KB cho 100 trạm

**Khi nào dùng:**
- Developer muốn nhúng bản đồ vào website
- Chia sẻ dữ liệu qua REST API
- Tích hợp với ứng dụng web/mobile

##### 2. Excel (.xlsx)

**Cấu trúc bảng:**

| Mã trạm | Ưu tiên | Điểm tổng | Cách dân cư (m) | Cách trạm điện (m) | Kinh độ | Vĩ độ |
|---------|---------|-----------|-----------------|---------------------|---------|--------|
| EV-001 | CAO | 78.5 | 450 | 890 | 105.8542 | 21.0285 |
| EV-002 | TRUNG BÌNH | 65.2 | 1200 | 650 | 105.8601 | 21.0312 |

**Ưu điểm:**
- ✅ **Phân tích dữ liệu**: Pivot table, biểu đồ, filter
- ✅ **Báo cáo quản lý**: Trình bày cho sếp, stakeholders
- ✅ **Tính toán bổ sung**: Thêm cột ngân sách, ROI, timeline
- ✅ **Không cần GIS**: Người không chuyên vẫn đọc được

**Khi nào dùng:**
- Quản lý dự án muốn báo cáo lên cấp trên
- Phân tích chi phí, lập kế hoạch triển khai
- Chia sẻ với bộ phận tài chính, hành chính

##### 3. Shapefile (.zip)

**Nội dung file zip:**
```
optimal_stations.zip
├── optimal_stations.shp  (geometry - hình học)
├── optimal_stations.shx  (index - chỉ mục)
├── optimal_stations.dbf  (attributes - thuộc tính)
├── optimal_stations.prj  (projection: WGS84)
└── optimal_stations.cpg  (encoding: UTF-8)
```

**Ưu điểm:**
- ✅ **GIS chuyên nghiệp**: Import trực tiếp vào QGIS/ArcGIS
- ✅ **Phân tích không gian**: Buffer, overlay, network analysis
- ✅ **Chuẩn ngành**: Tiêu chuẩn GIS quốc tế (ISO 19125)
- ✅ **Tích hợp dữ liệu**: Merge với các layer khác

**Khi nào dùng:**
- GIS analyst cần phân tích sâu
- Tính toán vùng phủ sóng, catchment area
- Phân tích giao thông, network coverage
- Tích hợp với hệ thống GIS hiện có

#### 📊 So sánh nhanh

| Tiêu chí | GeoJSON | Excel | Shapefile |
|----------|---------|-------|-----------|
| **Đối tượng** | Developer | Quản lý | GIS Analyst |
| **Công cụ mở** | Text editor, Web | Excel, Sheets | QGIS, ArcGIS |
| **Kích thước** | Nhẹ (~50KB) | TB (~100KB) | Nặng (~500KB) |
| **Dễ đọc** | Medium | Dễ nhất ⭐ | Khó (cần SW) |
| **Phân tích GIS** | Hạn chế | Không | Mạnh nhất ⭐ |
| **Chia sẻ web** | Tốt nhất ⭐ | Medium | Không phù hợp |

#### 💡 Gợi ý sử dụng

**Scenario 1: Báo cáo cho giám đốc (không biết GIS)**
→ Xuất **Excel** ✅

**Scenario 2: Nhúng vào website công ty**
→ Xuất **GeoJSON** ✅

**Scenario 3: Phân tích chi tiết trong QGIS**
→ Xuất **Shapefile** ✅

**Scenario 4: Chia sẻ đa mục đích**
→ Xuất **cả 3 định dạng** 🎯
